...
describe CouchbaseOrm::Associations do
  describe 'belongs_to' do
    it 'works with dependent associations' do
      ...
      expect(Parent.find_by_id(id)).to be_nil
    ...
    it 'ignores associations when delete is used' do
      parent = Parent.create!(name: 'joe')
      child = Child.create!(name: 'bob', parent_id: parent.id)
    ...
    it 'raises an error if an invalid type is being assigned' do
  ...
  describe 'has_and_belongs_to_many' do
    ...
    it 'adds association with single' do
      assembly = Assembly.create!(name: 'a1')
      part = Part.create!(name: 'p1', assemblies: [assembly])

      expect(assembly.reload.parts.map(&:id)).to contain_exactly(part.id)
    end

    it 'adds association with multiple' do
      assembly = Assembly.create!(name: 'a1')
      part1 = Part.create!(name: 'p1', assemblies: [assembly])
      part2 = Part.create!(name: 'p2', assemblies: [assembly])

      expect(assembly.reload.parts.map(&:id)).to contain_exactly(part1.id, part2.id)
    end

    it 'removes association with single' do
      ...
      expect(assembly2.reload.parts.map(&:id)).to contain_exactly(part.id)
    end

    it 'removes association with multiple' do
      assembly1 = Assembly.create!(name: 'a1')
      assembly2 = Assembly.create!(name: 'a2')
      ...
      expect(assembly1.reload.parts.map(&:id)).to contain_exactly(part1.id)
      expect(assembly2.reload.parts.map(&:id)).to contain_exactly(part1.id, part2.id)
    end
