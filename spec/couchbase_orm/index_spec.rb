...
describe CouchbaseOrm::Index do
  ...
  it 'prevents models being created if they should have unique keys' do
  ...
  it 'provides helper methods for looking up the model' do
    joe = IndexTest.create!(email: 'joe@aca.com')
  ...
  it 'cleans up itself if dangling keys are left' do
    joe = IndexTest.create!(email: 'joe@aca.com')
  ...
  it 'works with nil values' do
    ...
    joe.email = nil
    joe.save!
    expect(IndexTest.find_by_email('joe@aca.com')).to be_nil
    expect(IndexTest.find_by_email(nil)).to eq(joe)
  ...
  it "does not overwrite index's that do not belong to the current model" do
    ...
    joe.email = nil
    joe.save!
    expect(NoUniqueIndexTest.find_by_email('joe@aca.com')).to be_nil
