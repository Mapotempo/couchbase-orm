...
describe CouchbaseOrm::Types::Timestamp do
  it 'casts an integer to time' do
    ...
    expect(described_class.new.cast(t.to_i)).to eq(t)
  end

  it 'casts an integer string to time' do
    t = Time.at(Time.now.to_i)
    expect(described_class.new.cast(t.to_s)).to eq(t)
  end
end
...
describe CouchbaseOrm::Types::Date do
  it 'casts an string to date' do
    d = Date.today
    expect(described_class.new.cast(d.to_s)).to eq(d)
  end

  it 'serializes date to string' do
    d = Date.today
    expect(described_class.new.serialize(d)).to eq(d.to_s)
  end

  it 'gets the type from the registry' do
    expect(ActiveModel::Type.lookup(:date)).to eq(described_class.new)
  end
end
...
describe CouchbaseOrm::Base do
  ...
  it 'is able to set attributes with a hash with indifferent access' do
    t = TypeTest.new(ActiveSupport::HashWithIndifferentAccess.new(name: 'joe', age: 20, size: 1.5, renewal_date: Date.today,
                                                                  subscribed_at: Time.now, active: true))
    t.save!
  ...
  it 'is consistent with active record on failed cast' do
    ...
    expect(t.renewal_date).to be_nil
    expect(t.subscribed_at).to be_nil
    expect(t.active).to be true
  end
  ...
  it 'is able to query by array value' do
    ...
    expect(N1qlTypeTest.find_by_name(%w[laura joe]).to_a).to match_array [t, t2]
  end
  ...
  it 'is able to query by date (n1ql)' do
  ...
  it 'is able to query by custom type (n1ql)' do
    now = Time.now
