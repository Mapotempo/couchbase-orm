...
describe CouchbaseOrm::Types::Encrypted, skip: 'deprecated, to rework it' do
  ...
  def expect_serialized_secret(obj)
    expect(obj.send(:serialized_attributes)['encrypted$secret']).to eq({alg: 'CB_MOBILE_CUSTOM',
                                                                        ciphertext: base64_secret})
    expect(obj.send(:serialized_attributes)).not_to have_key 'secret'
    expect(obj.send(:serialized_attributes)['encrypted$secret2']).to eq({alg: 'CB_MOBILE_CUSTOM',
                                                                         ciphertext: 'a secret'})
    expect(obj.send(:serialized_attributes)).not_to have_key 'secret2'
  ...
  it 'prefix with custom algo' do
    ...
    expect(obj.send(:serialized_attributes)).not_to include 'secret'
  end
