...
module CouchbaseOrm
  module Types
    class Nested < ActiveModel::Type::Value
      ...
      def initialize(type:)
        raise ArgumentError.new('type is nil') if type.nil?
        raise ArgumentError.new("type is not a class : #{type.inspect}") unless type.is_a?(Class)
      ...
      def cast(value)
        ...
        raise ArgumentError.new("Nested: #{value.inspect} (#{value.class}) is not supported for cast")
      end
      ...
      def serialize(value)
        ...
        raise ArgumentError.new("Nested: #{value.inspect} (#{value.class}) is not supported for serialization")
      end
    end
  end
end
