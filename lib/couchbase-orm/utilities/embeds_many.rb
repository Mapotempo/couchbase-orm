  ...
  module EmbedsMany
    def embeds_many(name, class_name: nil)
      ...
      define_method("#{name}=") do |val|
        embedded_objects = val.map do |v|
          obj = v.is_a?(class_name) ? v : class_name.new(v)
          obj.instance_variable_set(:@_embedded, true)
          obj
        end

        serialized = embedded_objects.map do |v|
          v.serialized_attributes.merge(type: self.class.design_document)
        end

        self.write_attribute(name, serialized)
        self.instance_variable_set(instance_var, embedded_objects)
      end
