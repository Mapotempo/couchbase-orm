  ...
  module Index
    ...
    def index(attrs, name = nil, presence: true, &processor)
      ...
      define_singleton_method(processor_method) do |*values|
        values = attrs.zip(values).map { |attr, value|
          attribute_types[attr.to_s].serialize(attribute_types[attr.to_s].cast(value))
        }
        if processor
      ...
      define_singleton_method(find_by_method) do |*values|
        ...
        CouchbaseOrm.logger.debug {
          "#{find_by_method}: #{class_bucket_key_method} with values #{values.inspect} give key: #{key}"
        }
