  ...
  module Index
    ...
    def index(attrs, name = nil, presence: true, &processor)
      ...
      define_singleton_method(class_bucket_key_method) do |*values|
        processed = send(processor_method, *values)
      ...
      define_method(bucket_key_method) do |_args = nil|
        self.class.send(class_bucket_key_method, *send(bucket_key_vals_method))
      end
      ...
      define_singleton_method(processor_method) do |*values|
        ...
        if processor
          yield(values.length == 1 ? values.first : values)
        else
          values.join('-')
        end
      end
      ...
      define_method("#{name}_unique?") do
        values = send(bucket_key_vals_method)
      ...
      before_destroy do |record|
        ...
        record.class.collection.remove(record.send(bucket_key_method), cas: check_ref_id.cas) if check_ref_id && check_ref_id.content == record.id
        true
      end
