      ...
      def belongs_to(name, **options)
        ...
        define_method(name) do
          ...
          val = if options[:polymorphic]
                  ::CouchbaseOrm.try_load(send(ref))
                else
                  assoc.constantize.find(send(ref), quiet: true)
                end
        ...
        define_method(:"#{name}=") do |value|
          if value
            unless options[:polymorphic]
              ...
              if klass.design_document != value.class.design_document
                raise ArgumentError,
                      "type mismatch on association: #{klass.design_document} != #{value.class.design_document}"
              end
            end
            send(ref_ass, value.id)
          else
            send(ref_ass, nil)
          end
        ...
        define_method(:"#{name}=") do |value|
          if value
            unless options[:polymorphic]
              klass = assoc.constantize
              value.each do |v|
                if klass.design_document != v.class.design_document
                  raise ArgumentError,
                        "type mismatch on association: #{klass.design_document} != #{v.class.design_document}"
                end
              end
            end
            send(ref_ass, value.map(&:id))
          else
            send(ref_ass, nil)
          end
