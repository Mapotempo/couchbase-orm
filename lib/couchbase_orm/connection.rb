  ...
  class Connection
    ...
    def self.config
      @@config || {
        ...
        username: ENV.fetch('COUCHBASE_USER', nil),
        password: ENV.fetch('COUCHBASE_PASSWORD', nil),
        bucket: ENV.fetch('COUCHBASE_BUCKET', nil),
      }
    end
    ...
    def self.cluster
      @cluster ||= begin
        ...
        cb_config.connection_string = config[:connection_string] || raise(CouchbaseOrm::Error,
                                                                          'Missing CouchbaseOrm connection string')
