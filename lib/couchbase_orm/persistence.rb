...
module CouchbaseOrm
  module Persistence
    ...
    def save(**options)
      ...
      new_record? ? _create_record(**options) : _update_record(**options)
    end
    ...
    def save!(**options)
      self.class.fail_validate!(self) unless save(**options)
      self
    end
    ...
    def _update_record(*_args, with_cas: false, **options)
      ...
      run_callbacks :update do
        run_callbacks :save do
          ...
          resp = self.class.collection.replace(id, serialized_attributes.except(:id).merge(type: self.class.design_document),
                                               Couchbase::Options::Replace.new(**options))
    ...
    def _create_record(*_args, **options)
      ...
      run_callbacks :create do
        run_callbacks :save do
          assign_attributes(id: self.class.uuid_generator.next(self)) unless id
          CouchbaseOrm.logger.debug { "_create_record - Upsert #{id} #{serialized_attributes.to_s.truncate(200)}" }

          resp = self.class.collection.upsert(id, serialized_attributes.except(:id).merge(type: self.class.design_document),
                                              Couchbase::Options::Upsert.new(**options))
